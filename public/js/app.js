...
const API = {
  ...
  async deleteNomination(year, id) {
    const r = await fetch(
      API.base + "/nominations?year=" + year + "&id=" + id,
      {
        method: "DELETE",
        headers: API.headers(true),
      },
...
function launchConfetti() {
  ...
  var style =
    document.getElementById("confetti-style") ||
...
function renderYearNav() {
  ...
  state.years.forEach(function (y) {
    ...
    if (state.currentYear && state.currentYear.year === y.year)
      classes += " active";
...
function renderYearView() {
  ...
  badge.textContent =
    y.status === "complete" ? "Complete" : "Accepting Nominations";
...
function renderNominees() {
  ["song", "album", "concert", "rookie"].forEach(function (cat) {
    ...
    var catNoms = state.nominations.filter(function (n) {
    ...
    catNoms.forEach(function (nom) {
      ...
      info.innerHTML =
        '<div class="nominee-primary">' +
        crown +
        escHtml(getNomineePrimaryText(nom)) +
        '</div><div class="nominee-secondary">' +
        escHtml(getNomineeSecondaryText(nom)) +
        "</div>";
...
document
  .getElementById("add-nomination-btn")
  .addEventListener("click", function () {
...
document
  .getElementById("nom-category")
  .addEventListener("change", function (e) {
    document.getElementById("nom-fields").innerHTML = buildNomFields(
      e.target.value,
    );
  });
...
/* ─── WINNERS MODAL ──────────────────────────────────────────── */
document
  .getElementById("select-winners-btn")
  .addEventListener("click", function () {
      ...
      if (catNoms.length === 0) {
        ...
        catNoms.forEach(function (nom) {
          ...
          item.innerHTML =
            ...
            escHtml(getNomineePrimaryText(nom)) +
            '<div class="winner-radio-sublabel">' +
            escHtml(getNomineeSecondaryText(nom)) +
            "</div></div>";
...
document
  .getElementById("winners-cancel")
  .addEventListener("click", function () {
    document.getElementById("winners-modal").classList.add("hidden");
  });

document
  .getElementById("winners-submit")
  .addEventListener("click", async function () {
    ...
    for (var i = 0; i < cats.length; i++) {
      ...
      var sel = document.querySelector(
        'input[name="winner-' + cat + '"]:checked',
      );
      if (!sel) {
        errEl.textContent =
          "Please select a winner for " + CATEGORY_LABELS[cat];
        errEl.classList.remove("hidden");
        return;
      }
    ...
    if (!confirm("This will lock the year and announce winners. Continue?"))
      return;
...
function buildPresCategories() {
  return ["song", "album", "concert", "rookie"].filter(function (cat) {
    return state.nominations.some(function (n) {
      return n.category === cat;
    });
  });
}
...
function showPresNominees() {
  ...
  var noms = state.nominations.filter(function (n) {
    return n.category === cat;
  });
  document.getElementById("pres-category-label").textContent =
    CATEGORY_LABELS[cat];
  ...
  setTimeout(function () {
    ...
    if (overlay && btn) {
      var targetY = btn.offsetTop - overlay.clientHeight / 2;
      var startY = overlay.scrollTop;
      var distance = targetY - startY;
      var duration = 4000; // scroll speed in milliseconds, higher = slower
      var startTime = null;
      ...
      function scrollStep(timestamp) {
        ...
        overlay.scrollTop = startY + distance * ease;
...
function showPresWinner() {
  var cat = pres.categories[pres.catIndex];
  var winnerId = state.winners[cat];
  var winner = state.nominations.find(function (n) {
    return n.id === winnerId;
  });
  ...
  document.getElementById("pres-winner-name").textContent =
    getNomineePrimaryText(winner);
  document.getElementById("pres-winner-detail").textContent =
    getNomineeSecondaryText(winner);

  launchConfetti();

  var hasNext = pres.catIndex < pres.categories.length - 1;
  document.getElementById("pres-next-btn").textContent = hasNext
    ? "Next Category >"
    : "Finish >";
  pres.phase = "winner";
}
...
document
  .getElementById("start-presentation-btn")
